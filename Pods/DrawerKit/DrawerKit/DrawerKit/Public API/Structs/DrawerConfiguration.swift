...
public enum ContainerViewDimMode: Equatable {
    case clear, with(color: UIColor)

    public static func ==(lhs: ContainerViewDimMode, rhs: ContainerViewDimMode) -> Bool {
        switch (lhs,rhs) {
        case (.clear, .clear):
            return true
        case (.clear, .with(color: _)), (.with(color: _), .clear):
            return false
        case (.with(let lcolor), .with(let rcolor)):
            return lcolor == rcolor
        }
    }

    func color(for state: DrawerState) -> UIColor {
        if state == .collapsed {
            return .clear
        } else {
            switch self {
...
public struct DrawerConfiguration {
    ...
    public var containerViewDimMode: ContainerViewDimMode
    ...
    public init(totalDurationInSeconds: TimeInterval = 0.4,
                ...
                containerViewDimMode: ContainerViewDimMode = .clear,
        ...
        self.containerViewDimMode = containerViewDimMode
...
extension DrawerConfiguration: Equatable {
    public static func ==(lhs: DrawerConfiguration, rhs: DrawerConfiguration) -> Bool {
        return lhs.totalDurationInSeconds == rhs.totalDurationInSeconds
            ...
            && lhs.containerViewDimMode == rhs.containerViewDimMode
